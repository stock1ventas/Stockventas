...
# ══════════════════════════════════════════════════════
#  WEBHOOK DE CREEM (activación automática al pagar)
# ══════════════════════════════════════════════════════
@app.post("/webhook/creem")
def webhook_creem():
    """
    Creem llama a este endpoint cuando alguien paga.
    Configurar en Creem: Settings → Webhooks → Add Webhook
    URL: https://stockventas-production.up.railway.app/webhook/creem
    Eventos: checkout.completed, subscription.active
    """
    import hmac as hmac_lib
    import hashlib

    # Verificar firma de Creem
    CREEM_SECRET = os.environ.get("WEBHOOK_SECRET", "")
    signature    = request.headers.get("creem-signature", "")
    body         = request.get_data()

    if CREEM_SECRET and signature:
        expected = hmac_lib.new(
            CREEM_SECRET.encode(), body, hashlib.sha256
        ).hexdigest()
    ...
    if event_type not in ("checkout.completed", "subscription.active", "payment.succeeded"):
        return jsonify({"ok": True, "ignored": event_type})

    # Extraer email y producto del payload de Creem
    obj        = data.get("object", data.get("data", {}))
    email      = obj.get("customer_email") or obj.get("email", "")
    product_id = obj.get("product_id") or obj.get("items", [{}])[0].get("product_id", "")
    payment_id = obj.get("id", "")

    if not email:
        return jsonify({"error": "Email no encontrado en el payload"}), 400

    # Determinar el plan según el producto de Creem
    plan_key = "monthly"
    for key, plan in PLANS.items():
        if plan["creem_id"] == product_id:
            plan_key = key
            break
    ...
    if not user:
        db.close()
        # El usuario no existe todavía (pagó antes de registrarse)
        # Guardamos el pago pendiente para activar cuando se registre
        return jsonify({"ok": True, "note": "Usuario no encontrado, se activará al registrarse"})
    ...
    db.execute("""
        ...
        ON CONFLICT(user_id) DO UPDATE SET
          ...
          provider='creem', updated=excluded.updated
    """, (user["id"], plan_key, paid_until, payment_id, now))

    db.execute("UPDATE users SET plan=?, active=1 WHERE id=?", (plan_key, user["id"]))
    db.commit()
    log_access(user["id"], f"creem_payment:{plan_key}")
    db.close()

    return jsonify({"ok": True, "plan": plan["name"], "paid_until": paid_until})
...
PLANS = {
    "monthly": {"name": "Plan Completo Mensual", "price_usd": 30,  "days": 30,  "creem_id": "prod_7ZAu6Utmkr2kRr5TrDEd9L"},
    "annual":  {"name": "Plan Completo Anual",   "price_usd": 300, "days": 365, "creem_id": "prod_7XoSneogTter9kJDcZlNTR"},
}

CREEM_MONTHLY_URL = "https://www.creem.io/payment/prod_7ZAu6Utmkr2kRr5TrDEd9L"
CREEM_ANNUAL_URL  = "https://www.creem.io/payment/prod_7XoSneogTter9kJDcZlNTR"
